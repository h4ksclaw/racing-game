...
export class TireTemps extends LitElement {
	static override styles = [
		...
		css`
			...
			.tire-val.cold { color: rgba(59,130,246,0.6); }
			.tire-val.opt { color: var(--ui-green); }
			.tire-val.hot { color: var(--ui-amber); }
			.tire-val.over { color: var(--ui-red); }
		`,
	];
	...
	override render() {
		return html`
			...
			<div class="axle">
				${this.tire("FL", this.fl)}
				${this.tire("FR", this.fr)}
			</div>
			<div class="axle">
				${this.tire("RL", this.rl)}
				${this.tire("RR", this.rr)}
			</div>
		`;
	}
}
