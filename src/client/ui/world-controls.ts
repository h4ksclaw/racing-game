...
export class WorldControls extends LitElement {
	static styles = css`
		:host {
			...
			--panel-bg: rgba(10, 12, 18, 0.75); --panel-border: rgba(255,255,255,0.08);
			--panel-radius: 12px; --text-primary: #f0f2f5; --text-dim: rgba(240,242,245,0.55);
			--accent: #00e5a0; --accent-dim: rgba(0,229,160,0.15); --warning: #ff6b35;
		...
		.panel {
			...
			padding: 20px; width: 220px; color: var(--text-primary);
		}
		.title { font-size: 15px; font-weight: 600; margin-bottom: 14px; }
		.label { display: block; font-size: 11px; color: var(--text-dim); text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 6px; }
		.seed-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; }
		.seed-value { font-size: 13px; font-family: "SF Mono", "Cascadia Code", "Consolas", monospace; color: var(--accent); }
		...
		select {
			width: 100%; padding: 6px 10px; background: var(--input-bg); border: 1px solid var(--input-border);
			border-radius: 6px; color: var(--text-primary); font-size: 13px; outline: none; margin-top: 4px; cursor: pointer;
		}
		select option { background: #1a1c24; }
		.btn {
			display: block; width: 100%; padding: 9px 14px; border-radius: 8px; font-size: 13px;
			font-weight: 500; cursor: pointer; transition: all 0.15s ease; text-align: center; border: 1px solid; margin-bottom: 8px;
		}
		.btn-primary { background: var(--accent-dim); border-color: rgba(0,229,160,0.25); color: var(--accent); }
		.btn-primary:hover { background: rgba(0,229,160,0.25); border-color: rgba(0,229,160,0.4); }
		.btn-secondary { background: var(--input-bg); border-color: var(--input-border); color: var(--text-dim); }
		.btn-secondary:hover { background: rgba(255,255,255,0.1); color: var(--text-primary); }
		.btn-drive { background: rgba(255,107,53,0.12); border-color: rgba(255,107,53,0.25); color: var(--warning); }
		.btn-drive:hover { background: rgba(255,107,53,0.22); border-color: rgba(255,107,53,0.4); }
	...
	render() {
		const timeStr = this._fmt(this.hour);
		return html`
			<div class="panel">
				...
				<div class="field">
					...
					<input type="range" min="0" max="24" step="0.1" .value=${String(this.hour)} @input=${this._hr} />
				</div>
				<div class="field">
					<span class="label">Weather</span>
					<select .value=${this.weather} @change=${this._wth}>
	...
	private _gen() {
		...
		this.seed = s;
		this.dispatchEvent(
			new CustomEvent("generate", { detail: { seed: s }, bubbles: true, composed: true }),
		);
	}
	private _fly() {
	...
	private _prac() {
		...
		p.set("seed", String(this.seed));
		p.set("hour", String(this.hour));
		p.set("weather", this.weather);
		window.location.href = `/practice?${p.toString()}`;
	}
}
